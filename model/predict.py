...
_model = None       # ada underscore maksudnya dia private
...
def get_model():            # namanya lazy model
    global _model
    if _model is None:      # tidak perlu load kalau uda ada cek begini
        _model = pickle.load(open('model/artifacts/model_v1.sav', 'rb'))
        return _model
    return _model

def get_prediction(data:List[dict], proba=False) -> list:
